...
REPOS_MVN="release aii CAF cdp-listend configuration-modules-core configuration-modules-grid LC ncm-cdispd ncm-query"
    ...
    if gpg --yes --sign $0; then
        ...
        if [[ $prompt == "yes" ]]; then
            for r in $REPOS_MVN; do
                ...
                cd $r
                #mvn-c -q -DautoVersionSubmodules=true -B -DreleaseVersion=$VERSION clean release:prepare
                mvn_release
                if [[ $? -gt 0 ]]; then
